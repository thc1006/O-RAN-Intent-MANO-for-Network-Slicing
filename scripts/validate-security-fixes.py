...
def check_service_account_token(docs, component_name):
    ...
    if token_mount is True:
        return True, f"PASS {component_name}: automountServiceAccountToken correctly set to true"
    else:
        return False, f"FAIL {component_name}: automountServiceAccountToken should be true, got {token_mount}"
...
def check_image_reference(docs, component_name):
    ...
    if ':v1.0.1' in image and '@sha256:' not in image:
        return True, f"PASS {component_name}: Using proper versioned tag without placeholder digest"
    elif '@sha256:' in image and len(image.split('@sha256:')[1]) == 64:
        return True, f"PASS {component_name}: Using proper SHA256 digest"
    else:
        return False, f"FAIL {component_name}: Image reference needs proper versioning: {image}"
...
def check_image_pull_policy(docs, component_name):
    ...
    if pull_policy == 'Always':
        ...
        return False, f"FAIL {component_name}: imagePullPolicy should be Always, got {pull_policy}"
...
def check_seccomp_profile(docs, component_name):
    ...
    pod_seccomp = pod_spec.get('securityContext', {}).get('seccompProfile', {}).get('type')
    ...
    container_seccomp = containers[0].get('securityContext', {}).get('seccompProfile', {}).get('type')

    if pod_seccomp == 'RuntimeDefault' and container_seccomp == 'RuntimeDefault':
        return True, f"PASS {component_name}: Seccomp profiles correctly set to RuntimeDefault"
    else:
        return False, f"FAIL {component_name}: Seccomp profiles should be RuntimeDefault, got pod:{pod_seccomp}, container:{container_seccomp}"
...
def check_security_annotations(docs, component_name):
    ...
    pod_annotations = deployment.get('spec', {}).get('template', {}).get('metadata', {}).get('annotations', {})
...
def check_rbac_configuration(rbac_docs):
    ...
    for sa in service_accounts:
        ...
        if name in ['oran-orchestrator', 'oran-vnf-operator']:
            ...
            if token_mount is True:
                results.append((True, f"PASS ServiceAccount {name}: automountServiceAccountToken correctly set to true"))
            else:
                results.append((False, f"FAIL ServiceAccount {name}: automountServiceAccountToken should be true"))

            if 'security.policy/api-access-reason' in annotations:
                results.append((True, f"PASS ServiceAccount {name}: Security justification annotation present"))
            else:
                results.append((False, f"FAIL ServiceAccount {name}: Missing security justification annotation"))
...
def main():
    ...
    if not all([orchestrator_file.exists(), vnf_operator_file.exists(), rbac_file.exists()]):
